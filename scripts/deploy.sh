...
if [ -z "$GITHUB_TOKEN" ]; then
  ...
  githubUrl=git@github.com:jonsam-ng/oxygen-space.git
else
  msg='deploy: auto deploy by github actions'
  githubUrl=https://jonsam-ng:${GITHUB_TOKEN}@github.com/jonsam-ng/oxygen-space.git
fi
git init
git add -A
git commit -m "${msg}"
